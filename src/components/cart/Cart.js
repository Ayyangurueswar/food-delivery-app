...
import {
    addItemToCart, removeItemFromCart, updateCartQuantity, 
...
const Cart = () => {
  ...
  const increaseQty = (id, quantity, stock) => {
    ...
    dispatch(addItemToCart(id, newQty)).then(()=>{
      alert.success("Item added");
    }).catch((error) => {
      alert.info("Item can't be added");
    });
  }
  ...
  const decreaseQty = (id, quantity) => {
    if(quantity > 1){
        const newQty = quantity - 1;
        dispatch(updateCartQuantity(id, newQty)).then(()=>{
          alert.success("Item removed");
        }).catch((error) => {
          alert.info("Item can't be removed");
        });
    }
    else if(quantity === 1){
                          ...
                          <div className='cart-item' key={item.fooditem}>
                            <div className='row'>
                              ...
                              <div className='col-4 col-lg-3 mt-4 mt-lg-0'>
                                <div className='stockCounter inline'>
                                  <span className='btn btn-danger minus' 
                                  onClick={() => {decreaseQty(item.fooditem, item.quantity)}}>-</span>
                                  <input type='number' 
                                  ...
                                  onClick={() => {increaseQty(item.fooditem, item.quantity)}}>+</span>
                                </div>
                              </div>
